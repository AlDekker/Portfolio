...
function createServer() {
  browserSync.init({
    proxy: 'http://localhost:8888/steamtrade/',
    notify: false,
  })
}
...
function rsyncDeploy() {
  ...
  .pipe(rsync({
    root: distFolder,
    hostname: 'webbranchy@77.222.62.59',
    destination: '/home/w/webbranchy/test/currentwork1/public_html',
    recursive: true,
    archive: true,
    silent: false,
    compress: true
...
export { styles, scripts, images, imagesWebp, sprite, buildDist, clearBuild, rsyncDeploy}
export let build = series(clearBuild, parallel(styles, scripts, sprite), buildDist, images)
export let deploy = series(build, rsyncDeploy)
